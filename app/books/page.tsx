...
export default function BooksPage() {
  ...
  return <main className="books-page">
    <header className="books-nav"><a className="brand" href="/"><span className="brand-symbol"><img src="/logo-encoding-tree.svg" alt="" /></span><b>MACHINE<br />INTELLIGENCE</b></a><a className="back-link" href="/">{c.back}</a><button onClick={() => setLang(lang === "zh" ? "en" : "zh")}>{c.toggle}</button></header>
    ...
    <section className="book-index shell">{c.books.map((book, index) => <article id={["artificial-intelligence-science", "artificial-intelligence-principles", "sun-tzu-ai-principles"][index]} className="book-index-card" key={book.title}><div className="book-index-top"><span>{book.year}</span><span>{book.kind}</span></div><h2>{book.title.split("\n").map(line => <span key={line}>{line}</span>)}</h2><p className="book-index-en">{book.enTitle}</p><p className="book-index-meta">{book.meta}</p><p className="book-index-summary">{book.summary}</p><div className="book-parts">{book.parts.map((part) => <span key={part}>{part}</span>)}</div><div className="book-links">{book.links.map(([label, href]) => <a className="button light book-download" href={href} target={href.startsWith("/") ? undefined : "_blank"} rel={href.startsWith("/") ? undefined : "noreferrer"} key={label}>{label} ↗︎</a>)}</div></article>)}</section>
