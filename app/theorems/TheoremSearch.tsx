  ...
  return <main className="theorem-index">
    ...
    <section className="theorem-results theorem-shell">
      ...
      <ol>{filtered.slice(0, limit).map((entry) => <li key={entry.id}>
        ...
        <article><p>{entry.bookTitle}</p><h2>{entry.title}</h2><div>{entry.content}</div><footer><span>{entry.printedPage ? `原书第 ${entry.printedPage} 页` : `PDF 第 ${entry.pdfPage} 页`}</span><a href={entry.href}>查看出处 ↗</a></footer></article>
      </li>)}</ol>
