...
export default function AIPrinciplesPage() {
  return <PrinciplePage
    zh={{
      ...
      sections: [["从计算到谋算", "神经网络、大模型与世界模型不应只是不可解释的工具；它们需要被放进信息科学原理中，成为可以观察、解码、推演和验证的系统。"], ["从原理到机器", "自我意识机器、孙子模型、谋算智能机器与谋算智能机器人，是把数学原理落实到智能实体的四类研究载体。"], ["白盒智能", "白盒不是放弃性能，而是让智能的表示、决策、行动和验证过程具有结构依据，能够解释、控制和持续改进。"]],
    ...
    en={{
      ...
      sections: [["From computation to planning", "Neural networks, foundation models, and world models should not remain inexplicable tools. They belong inside information science: systems that can be observed, decoded, inferred, and verified."], ["From principles to machines", "Self-awareness machines, the Sun Tzu model, strategic-computational machines, and strategic-computational robots are four research carriers for bringing mathematical principles into intelligent entities."], ["White-box intelligence", "White-box does not mean giving up performance. It means that representation, decision, action, and verification have structural grounds and can be explained, controlled, and improved."]],
