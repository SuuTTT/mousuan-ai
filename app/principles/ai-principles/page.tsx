...
export default function AIPrinciplesPage() {
  return <PrinciplePage
    zh={{
      ...
      sections: [["从计算到谋算", "神经网络、大模型与世界模型不应只是不可解释的工具；它们需要被放进信息科学原理中，成为可以观察、解码、推演和验证的系统。"], ["从原理到机器", "自我意识机器、孙子模型、谋算智能机与谋算双脑体系结构智能机，是把数学原理落实到智能实体的研究载体。"], ["智、能与智能", "“智”是认知与策略设计的科学原理，即策略性设计；“能”是有效行动的工程原理与执行能力，即高效行动。智能是二者的统一，智能需要行动。"], ["谋算双脑", "谋脑负责层谱抽象的全局认知与推理，算脑负责分而治之的演算与推理；两脑协同，让机器同时具备理解整体、计算局部和有效行动的能力。"], ["白盒智能", "白盒不是放弃性能，而是让智能的表示、决策、行动和验证过程具有结构依据，能够解释、控制和持续改进。"]],
    ...
    en={{
      ...
      sections: [["From computation to MouSuan Strategy", "Neural networks, foundation models, and world models should not remain inexplicable tools. They belong inside information science: systems that can be observed, decoded, inferred, and verified."], ["From principles to machines", "Self-awareness machines, the Sun Tzu model, MouSuan intelligent machines, and MouSuan dual-brain architecture intelligent machines are research carriers for bringing mathematical principles into intelligent entities."], ["Zhi, Neng, and intelligence", "Zhi refers to the scientific principles of cognition and strategic design; Neng refers to the engineering principles and capability for effective action. Intelligence, in the Chinese sense, is their unity and requires action."], ["The MouSuan dual brain", "The Mou brain performs hierarchically abstracting global recognition and reasoning; the Suan brain performs divide-and-conquer calculation and reasoning. Together they connect whole-system understanding with local computation and effective action."], ["White-box intelligence", "White-box does not mean giving up performance. It means that representation, decision, action, and verification have structural grounds and can be explained, controlled, and improved."]],
