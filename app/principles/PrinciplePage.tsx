...
export function PrinciplePage({ zh, en, variant }: { zh: PrincipleCopy; en: PrincipleCopy; variant?: "application" }) {
  ...
  return <main className={`principle-page${variant ? ` ${variant}-page` : ""}`}>
    <header className="principle-nav"><a className="brand" href="/"><span className="brand-symbol"><img src="/logo-encoding-tree.svg" alt="" /></span><b>MACHINE<br />INTELLIGENCE</b></a><a className="principle-back" href="/">{c.back}</a><button onClick={() => setLang(lang === "zh" ? "en" : "zh")}>{c.toggle}</button></header>
