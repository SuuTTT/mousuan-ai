...
export default async function ThemePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="theme-page">
    ...
    <section className="theme-interpretation"><div className="theme-shell"><span>核心解释</span><p><MathText text={theme.interpretation} /></p></div></section>

    <section className="theme-structure theme-shell"><header><p>CORE STRUCTURE</p><h2>{theme.title}</h2></header><div>{theme.parts.map((part, partIndex) => <article key={part.title}><span>0{partIndex + 1} · {part.label}</span><h3>{part.title}</h3><p><MathText text={part.text} /></p></article>)}</div></section>

    {theme.informationLaws && <section id="information-world-laws" className="theme-information-laws"><div className="theme-shell"><header><p>TEN LAWS OF THE INFORMATION WORLD</p><h2>信息世界十大定律</h2></header><ol>{theme.informationLaws.map((law) => <li className="theme-information-law" key={law.no}><span>{law.no}</span><div><h3>{law.title}</h3><p><MathText text={law.text} /></p></div><a href={law.href}>{law.reference} ↗︎</a></li>)}</ol></div></section>}

    {theme.sunLaws && <section className="theme-sun-laws"><div className="theme-shell"><header><p>SUN TZU’S FIVE LAWS</p><h2>物质与信息结合的孙子五大定律</h2><span>这五大定律奠定了同时存在物质与信息的战争之科学原理。</span></header><ol>{theme.sunLaws.map((law) => <li key={law.no}><span>{law.no}</span><div><h3>{law.title}</h3><p><MathText text={law.text} /></p></div><a href={law.href}>原文 ↗︎</a></li>)}</ol></div></section>}

    <section className="theme-basis"><div className="theme-shell"><header><p>KEY BASIS</p><h2>关键依据</h2></header><div>{theme.basis.map((source, sourceIndex) => <article key={`${source.label}-${sourceIndex}`}><span>[{sourceIndex + 1}]</span><div><strong>{source.label}</strong>{source.references ? <ol>{source.references.map((reference) => <li key={reference.number}><a href={reference.href}><span>{reference.number}</span><h3>{reference.title}</h3><p><MathText text={reference.content} /></p></a></li>)}</ol> : <small>{source.detail}</small>}</div><a href={source.href} aria-label={`查看${source.label}`}>↗︎</a></article>)}</div></div></section>
